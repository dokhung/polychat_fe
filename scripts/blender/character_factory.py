...
def build(gender):
    ...
    for i in range(1,25):
        ...
        cheek=art_surfaces.jaw_factor(z)
    ...
    for s,suffix in [(1,'L'),(-1,'R')]:
        ...
        loft('TaperedPants',[(0.18,s*.106,0,.056,.059),(.20,s*.106,0,.061,.065),(.34,s*.106,0,.073,.079),(.47,s*.10,0,.088,.093),(.59,s*.095,0,.099,.107)],'Bottom','Bottom','leg.'+suffix,n=28)
        loft('TrouserCuff',[(.177,s*.106,0,.057,.060),(.196,s*.106,0,.061,.065)],'Bottom','Bottom','lower_leg.'+suffix,n=28)
        loft('Ankle',[(.126,s*.106,0,.05,.052),(.20,s*.106,0,.053,.056)],'Skin','Body','lower_leg.'+suffix,n=24)
    ...
    for part,objects in PARTS.items():
        ...
        if part!='Head':
            ...
            bpy.ops.object.join()
            objects[0].name=part
    # Mature SD silhouette: roughly three heads tall, with longer legs.
    # Apply the same map to the rest skeleton and every shape-key coordinate.
    def body_point(p):
        p=Vector(p)
        p.z=p.z*1.14 if p.z<.56 else p.z+.0784
        return p
    def head_point(p):
        p=Vector(p)
        p.x*=.90; p.y*=.90; p.z=1.575+.90*(p.z-1.518)
        return p
    head_parts={'Head','Eyes','Eyebrows','Mouth','HairFront','HairSide','HairBack'}
    for obj in bpy.context.scene.objects:
        if obj.type!='MESH': continue
        transform=head_point if obj.get('slot') in head_parts else body_point
        if obj.data.shape_keys:
            for key in obj.data.shape_keys.key_blocks:
                for vertex in key.data: vertex.co=transform(vertex.co)
        else:
            for vertex in obj.data.vertices: vertex.co=transform(vertex.co)
    bpy.context.view_layer.objects.active=rig
    bpy.ops.object.mode_set(mode='EDIT')
    for bone in rig.data.edit_bones:
        bone.head=head_point(bone.head) if bone.name=='head' else body_point(bone.head)
        bone.tail=head_point(bone.tail) if bone.name in {'head','neck'} else body_point(bone.tail)
    bpy.ops.object.mode_set(mode='OBJECT')
    height=max(v.co.z for obj in bpy.context.scene.objects if obj.type=='MESH' for v in obj.data.vertices)
    animations(rig)
