...
def jaw_factor(z):
    return (1+.020*math.exp(-((z-1.12)/.075)**2))*(1-.08*max(0,min(1,(1.20-z)/.24)))

def face_y(x,z):
    return -.245*math.sqrt(max(.015,1-(x/(.276*jaw_factor(z)))**2-((z-1.205)/.274)**2))
...
def face(api,female):
    for s in [-1,1]:
        ...
        patch(api,'Sclera',x,z,.064,.050,'White',offset=.002,bulge=.003)
        patch(api,'Iris',x-s*.004,z,.034,.046,'Eyes',offset=.008,bulge=.001)
        patch(api,'IrisGlow',x-s*.004,z-.023,.025,.022,'IrisWarm',offset=.010,bulge=.001,rings=4)
        patch(api,'Pupil',x-s*.004,z+.009,.022,.034,'Pupil',offset=.012,bulge=.001)
        patch(api,'EyeSpark',x-.012,z+.022,.008,.010,'Highlight',offset=.015,bulge=.001,rings=3,n=20)
        patch(api,'EyeSparkSmall',x+.014,z-.019,.003,.004,'Highlight',offset=.015,bulge=.001,rings=2,n=12)
        pts=[]
        for i in range(7):
            a=.08+ (math.pi-.16)*i/6
            px=x+.064*math.cos(a); pz=z+.050*math.sin(a)
        ...
        pts=[(x-.044,1.283),(x-.003,1.290),(x+.041,1.285)]
        api.sweep('Brow',[(px,face_y(px,pz)-.004,pz) for px,pz in pts],[.002,.006,.002],[.002,.003,.002],'Hair','Eyebrows',n=8)
        patch(api,'CheekTint',s*.169,1.122,.027,.009,'Blush','Head',offset=.001,bulge=0,rings=4)
    api.ellipsoid('Nose',(0,face_y(0,1.136)-.005,1.136),(.016,.014,.023),'Skin','Head',segments=20,rings=12)
    pts=[(-.035,1.078),(-.013,1.074),(.010,1.074),(.034,1.081)]
...
def hair(api,female):
    ...
    ends=[(-2.60,1.54),(-2.26,1.37),(-1.95,1.24),(-1.61,1.16),(-1.25,1.12),(-.87,1.30)]
